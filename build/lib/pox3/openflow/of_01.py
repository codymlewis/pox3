...
Because many POX3 applications use OpenFlow, this component gets some
...
import pox3
import pox3.lib.util
from pox3.lib.addresses import EthAddr
from pox3.lib.revent.revent import EventMixin
import datetime
import time
from pox3.lib.socketcapture import CaptureSocket
import pox3.openflow.debug
from pox3.openflow.util import make_type_to_unpacker_table
from pox3.openflow import *
  ...
  def handle_FEATURES_REPLY (self, con, msg):
    ...
    if nexus is None:
      ...
      con.info("No OpenFlow nexus for " +
              pox3.lib.util.dpidToStr(msg.datapath_id))
      con.disconnect()
      return
    con.ofnexus = nexus
...
class DeferredSender (threading.Thread):
  ...
  def __init__ (self):
    ...
    self._waker = pox3.lib.util.makePinger()
    self.sending = False
...
class Connection (EventMixin):
  ...
  def __str__ (self):
    ...
    if self.dpid is None:
      ...
      d = pox3.lib.util.dpidToStr(self.dpid)
...
from pox3.lib.recoco.recoco import *
...
class OpenFlow_01_Task (Task):
  ...
  def __init__ (self, port = 6633, address = '0.0.0.0',
    ...
    core.addListener(pox3.core.GoingUpEvent, self._handle_GoingUpEvent)
  ...
  def run (self):
    ...
    try:
      ...
      if errno == EADDRNOTAVAIL:
        ...
        log.error(" Use openflow.of_01 --port=<port> to run POX3 on "
                  "another port.")
      return
        ...
        while True:
          ...
          for con in rlist:
            if con is listener:
              ...
              if pox3.openflow.debug.pcap_traces:
