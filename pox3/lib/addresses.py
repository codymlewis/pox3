...
import sys
if 'long' not in sys.modules['builtins'].__dict__:
  long = int
  ...
  try:
    ...
    for line in f.readlines():
      ...
      if not '-' in split[0]:
        continue
      # grab 3-byte OUI
      oui  = b''.join(int(x,16).to_bytes(1, 'little') for x in split[0].split('-'))
  ...
  def __init__ (self, addr):
    ...
    if isinstance(addr, bytes) or isinstance(addr, str):
      if len(addr) == 6:
        ...
        addr = b''.join((int(addr[x*2:x*2+2], 16).to_bytes(1, "little") for x in range(0,6)))
      else:
