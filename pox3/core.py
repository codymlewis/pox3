...
class (an instance of which is available as pox3.core.core).
...
import pox3.lib.revent.revent
pox3.lib.revent.revent.handleEventException = _revent_exception_hook
...
import pox3.lib.recoco as recoco

class POXCore (EventMixin):
  """
  A nexus of of the POX API.

  pox3.core.core is a reference to an instance of this class.  This class
  ...
  there will then be a pox3.core.core.foo).  In many cases, this means you
  ...
  @property
  def banner (self):
    return "{0} / Copyright 2020 Cody Lewis, et al.".format(
     self.version_string)

  @property
  def version_string (self):
    return "POX3 %s (%s)" % ('.'.join(map(str,self.version)),self.version_name)
  ...
  def _add_signal_handlers (self):
    ...
    import threading
    if not isinstance(threading.current_thread(), type(threading.main_thread())):
  ...
  def goUp (self):
    ...
    if vers != "3.8":
      ...
      l.warn("POX3 requires Python 3.8. You're running %s.", vers)
      l.warn("If you run into problems, try using Python 3.8.")
  ...
  def register (self, name, component=None):
    """
    Makes the object "component" available as pox3.core.core.name.
